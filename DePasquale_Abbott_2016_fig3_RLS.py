...
def F_in_out(h,n_trials):
    inter_trial_min = 300 #ms
    inter_trial_max = 100 #ms
...
def solve_J_W(h,F_in):
    ...
    N_out = 1
    N_in  = 1
    U = np.random.uniform(-1,1,(N,N_in))
    mu = -40 
    g_f = 12 
    g = 10  
    J_f = np.random.normal(mu/N,(g_f^2)/N,(N,N))
    I = 10
    tau_s = 100
    tau_f = 2
    tau_m = 20
    ...
    N_tild = 1000
    tau_x = 10
    J_tild = np.random.normal(0,1/N_tild,(N_tild,N_tild))
    u_tild = np.random.uniform(-1,1,(N_tild,N_out))
    U_tild = np.random.uniform(-1,1,(N_tild,N_in))
    g_tild = 1.2
    x = np.random.uniform(0,1,(N_tild,1))
    u = np.random.uniform(-np.sqrt(3/N_tild),np.sqrt(3/N_tild),(N,N_tild))
    
    J = np.zeros((N,N))
    #firings = np.zeros((N,np.shape(t)[0]))
    for i in np.arange(np.shape(t)[0]): 
        fired = np.nonzero(V >= V_th)[0] 
        #firings[fired,i] = 1
        V[fired]=V_rest
        s[fired]=s[fired]+1
        f[fired]=f[fired]+1
        ...
        x = x + h*((-x+g_tild*J_tild@(np.tanh(x))+u_tild*F_out[i]+U_tild*F_in[i])/tau_x)
        F_J = u@(g_tild*J_tild@(np.tanh(x))+u_tild*F_out[i])
        
        if i  == 0: 
            AA_J = s@(s.transpose())
            Ay_J = s@(F_J.transpose())
        else:
            AA_J = AA_J + s@(s.transpose())
            Ay_J = Ay_J + s@(F_J.transpose())
        J = np.linalg.inv(AA_J)@Ay_J

        if i  == 0: 
            AA_W = s@(s.transpose())
            Ay_W = s*(F_out[i])
        else:
            AA_W = AA_W + s@(s.transpose())
            Ay_W = Ay_W + s*(F_out[i])
        W = np.linalg.inv(AA_W)@Ay_W
        
        if np.remainder(t[i],10) == 0:
            print(t[i])
    return J, J_f, U, W
        
start_time = time.time()
J, J_f, U, W = solve_J_W(h,F_in)
print("--- %s seconds ---" % (time.time() - start_time))
         
'''   
#  %matplotlib auto
plt.imshow(firings[0:500][:,0:i], cmap='gray')
'''
...
np.save(('fig3_h' + str(h*1000) + 'us_' + str(n_trials) + '_J'),J)
np.save(('fig3_h' + str(h*1000) + 'us_' + str(n_trials) + '_J_f'),J_f)
np.save(('fig3_h' + str(h*1000) + 'us_' + str(n_trials) + '_U'),U)
np.save(('fig3_h' + str(h*1000) + 'us_' + str(n_trials) + '_W'),W)
